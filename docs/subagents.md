...
- **An attached sandbox is forwarded.** A real parent sandbox, including a deferred connection, is passed to each sub-agent. When the parent has only Pydantic AI's default no-sandbox placeholder, the child run resolves its own sandbox capability instead. An application-supplied `UnavailableSandbox` remains unavailable.
