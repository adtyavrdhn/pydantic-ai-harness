...
When users differ in more than their credential, such as a platform acting on each user's Connect account, build the whole capability for each run with a [dynamic capability](/ai/capabilities/custom/#dynamically-building-a-capability):
