...
A direct `LLMReminder` entry in `dynamic_reminders` is a journaled capability operation under durability engines that support capability operations. Generation errors are recorded as a fallback result rather than inheriting the engine's retry policy, so a best-effort reminder cannot stall the run; replay restores that recorded result instead of repeating the model call. `SystemReminders` carries the stable default `id='system_reminders'`, so durable recovery works without configuration. Set an explicit `id` when attaching more than one `SystemReminders` instance to one agent, so each keeps its own operation identity. A wrapper such as `every_tenth` above calls `LLMReminder` from orchestration context and is not workflow-safe: engines that forbid I/O there can fail the call outright. An `LLMReminder` subclass that overrides `__call__` takes that same non-journaled route, because the override has to run. Use a direct `LLMReminder` entry for the durable-safe form. Without a durability engine, generation runs directly with the same fallback to `GoalReanchor` on error.
