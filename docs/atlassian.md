...
The function is called at the start of each run, so each run connects as its own user. It can return a token or an `httpx.Auth`, such as `httpx.BasicAuth(email, token)` for a user's personal API token. If it returns `None`, that run has no Atlassian tools; it never falls back to `ATLASSIAN_API_KEY`.

Your application is responsible for getting each user's token, storing it, and refreshing it, for example with a "Connect Atlassian" button in your web app. Look the token up before the run, for example with `await`, and put it in the deps; the function only reads it.
