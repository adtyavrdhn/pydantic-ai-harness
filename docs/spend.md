...
That pause holds a coroutine, so it lasts as long as the process does and no longer: there is no *serializable* pause at a model-request boundary. `CallDeferred` and `ApprovalRequired` are honored where a tool call is validated or executed; raised from a model-request hook, nothing catches them and the run ends on the bare exception, which carries no message of its own.
...
`defer_loading=True` is refused. A deferred capability's hooks do not run until the model loads it, so an exhausted budget would not stop a request and the requests made meanwhile would go uncounted.

`SpendLimits` accrues inside its own model-request wrapper. Requests another capability skips are never charged, and a retry of an already-counted response does not double-count.

What ordering cannot fix is a capability whose own model-request wrapper sits inside the accrual and rejects a response the provider has already billed: the counter then reads lower than what the provider charged. Before the first request `SpendLimits` reads the composed chain and emits `SpendCompositionWarning` naming capabilities listed after it that wrap model requests. The report is keyed on list position, so it can name a capability that in practice cannot under-count, and it does not see a bare `Hooks` instance or a durable-execution integration. Listing `SpendLimits` last among your innermost capabilities closes the gap where one exists and silences the warning.

**Durable execution.** `SpendLimits` is not supported inside a Temporal workflow, and only partially supported inside a DBOS or Prefect one.

The hooks accrue in orchestration code while the model request beside them is a durable unit. Recovery re-executes orchestration code but restores the model request from its checkpoint, so the accrual re-runs against a response the engine did not re-pay. What that does to the counter follows the store: one that reaches the recovery, a shared store or the in-process default when recovery lands in the same process, recognises the replayed response and counts it once; one that does not starts from nothing and admits spend it should refuse. Every failure mode errs the same way, a billed response counted twice rather than one dropped. Temporal surfaces the problem outright and fails the workflow on the first request; DBOS and Prefect recover silently.
...
Closing the gap fully needs the window key to come from a clock the engine controls and the store write to happen inside the engine's own durable unit. Neither can be arranged without depending on `temporalio`, `dbos` or `prefect` and detecting which one is running, so full durable-execution support stays out of scope for this capability.
