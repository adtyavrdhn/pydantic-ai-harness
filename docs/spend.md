...
`add_many` carries a token identifying the response, and `RedisSpendStore` reads a marker for it before the increments and writes it after them, inside the same script. `InMemorySpendStore` remembers the same tokens under its lock, so the default store behaves the same way. This deduplicates retries that reach the store outside a durability engine's journaled accrual operation. The token is the provider's own response id where the provider reports one; without one it falls back to the run id, the step, and the response timestamp. Markers are held for `dedup_retain`, a field on both stores, an hour by default, or for the window's own horizon where that is shorter, and cost one small key per response per window. Set `dedup_retain=None` to hold no markers and apply every entry.
...
Any object with `get_many` and `add_many` works, so a Postgres or DynamoDB counter is a small class rather than a fork. Four obligations come with writing one. Return a total for every key you were handed, keyed by `SpendEntry.key`, including one skipped because its token was already applied. Read a key that was never written as zero rather than leaving it out. Deduplicate tokens when direct store retries need that protection. Apply the whole call or none of it, so a failure cannot leave only some windows updated. Neither method is handed an empty sequence.
...
What is left is siblings. Pydantic AI orders innermost capabilities against non-innermost ones only, and among themselves the one listed *later* nests further in. `InputGuardrail` listed after `SpendLimits` can reject a billed response before it is counted. With `InputGuardrail(parallel=True)` what decides is whether the guard blocks, not who wins the race: a blocked prompt is counted in neither outcome, because the guard cancels the call when it settles first and discards the answer when the model does. List `SpendLimits` last among your innermost capabilities where the difference matters. Closing it outright needs a way to order innermost capabilities against each other, tracked in [#534](https://github.com/pydantic/pydantic-ai-harness/issues/534).
...
Three kinds of capability are left out of that report. A `Hooks` is not named: it defines `wrap_model_request` whether or not a `model_request` hook was registered, and the registry that would say is private ([pydantic-ai#7177](https://github.com/pydantic/pydantic-ai/issues/7177)). A `WrapperCapability` is answered on whatever it wraps, since its own `wrap_model_request` only delegates -- so a wrapper over a real rejector is still named. A durable-execution capability is also left out because routing into a durable unit does not reject the response.

**Durable execution.** `SpendLimits` journals its counter reads, clock reads, and accruals when a durability engine supports capability operations. The complete `add_many` call is one durable operation. Its returned totals are journaled with the operation, so replay reuses them instead of entering the store and adding the response again. This exactly-once boundary is required because the model request beside the accrual is also restored rather than repeated.

Store-level `SpendEntry.token` deduplication remains defense in depth for retries that reach the store outside an engine replay. It is not the primary replay boundary under a durability engine. The counter still needs storage shared by capability-operation recovery instances. `InMemorySpendStore` is not shared across those instances, so use a shared store for durable workflows and when budgets must coordinate concurrent runs.
...
You can also refuse workflow **admission** before starting it. That is why `exhausted()` works without a `RunContext`:
...
Admission only reads existing counters and reserves nothing. The durable operations inside the run perform the accounting.
