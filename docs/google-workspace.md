...
Pass `access='write'` to expose every tool for the selected products. Add `require_approval=True` to pause the run before each tool that is not read-only, then approve and resume it as the [deferred tools guide](/ai/deferred-tools/) describes:
