...
class TestCloudflare:
    ...
    def test_custom_client_owns_authentication(self) -> None:
        ...
        assert transport(Cloudflare(client=client)).auth is client.auth

    @pytest.mark.parametrize('settings', [{'auth': 'token'}, {'auth': no_credential}, {'server': CloudflareServer.API}])
...
class TestPerRunAuth:
    ...
    def test_fixed_oauth_uses_browser_login(self) -> None:
        assert isinstance(transport(Cloudflare(auth='oauth', server=CloudflareServer.API)).auth, OAuth)
