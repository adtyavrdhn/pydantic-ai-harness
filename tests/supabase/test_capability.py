...
class TestSupabase:
    ...
    def test_custom_client_owns_authentication(self) -> None:
        ...
        assert transport(Supabase(client=client)).auth is client.auth

    @pytest.mark.parametrize(
        'settings',
        [{'auth': 'key'}, {'auth': no_credential}, {'project_ref': 'abc123'}, {'features': ['database']}],
    )
...
class TestPerRunAuth:
    ...
    async def test_provider_returning_none_does_not_fall_back(
        ...
        monkeypatch.setenv('SUPABASE_ACCESS_TOKEN', 'deployment-token')
        capability = Supabase[str | None](auth=lambda ctx: ctx.deps)
