...
@dataclass
class _BoundTools:
    toolset: ModalSandboxToolset[None]
    ctx: RunContext[None]

    async def run_command(self, command: str, *, timeout_seconds: float | None = None) -> str:
        return await self.toolset.run_command(self.ctx, command, timeout_seconds=timeout_seconds)

    async def read_file(self, path: str, *, offset: int = 1, limit: int | None = None) -> str:
        return await self.toolset.read_file(self.ctx, path, offset=offset, limit=limit)

    async def write_file(self, path: str, content: str) -> str:
        return await self.toolset.write_file(self.ctx, path, content)

    async def list_directory(self, path: str = '.') -> str:
        return await self.toolset.list_directory(self.ctx, path)
...
async def _toolset(
    *,
    ...
    async with AsyncExitStack() as stack:
        ...
        sandbox = active_session._sandbox  # pyright: ignore[reportPrivateUsage]
        ...
        ctx = RunContext(
            ...
            usage=RunUsage(),
            run_id='compat-test',
            sandbox=Sandbox(ModalSandboxBackend(sandbox)),
        )
        yield _BoundTools(toolset, ctx)
...
class TestRunCommand:
    ...
    async def test_tool_on_unentered_toolset_raises(self) -> None:
        ...
        typed_toolset = cast(ModalSandboxToolset[None], toolset)
        with pytest.raises(ModalSandboxError, match='not active in an agent run'):
            await typed_toolset.run_command(_run_context(), 'echo hi')
...
class TestToolsetLifecycle:
    ...
    async def test_for_run_does_not_provision_before_a_tool_call(self, fake_modal: FakeModal) -> None:
    ...
    async def test_for_run_carries_the_session(self, fake_modal: FakeModal) -> None:
        async with ModalSandboxSession() as session:
            ...
            async with fresh:
                ...
                sandbox = session._sandbox  # pyright: ignore[reportPrivateUsage]
                assert sandbox is not None
                ctx = RunContext(
                    deps=None,
                    model=TestModel(),
                    usage=RunUsage(),
                    run_id='injected-session',
                    sandbox=Sandbox(ModalSandboxBackend(sandbox)),
                )
                await fresh.run_command(ctx, 'echo hi')
...
class TestCapability:
    ...
    async def test_agent_context_does_not_create_an_unused_base_sandbox(self, fake_modal: FakeModal) -> None:
        ...
        async with agent:
            ...
            assert len({sandbox.object_id for sandbox in fake_modal.sandboxes}) == 2
