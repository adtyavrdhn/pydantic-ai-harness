...
class ModalSandboxToolset(FunctionToolset[AgentDepsT]):
    ...
    Retains the released four-tool surface while executing against the sandbox supplied
    by the capability through `ctx.sandbox`. A lightweight attached session preserves the
    released bounded-output implementation without provisioning or owning another sandbox.
    """
    ...
    async def for_run(self, ctx: RunContext[AgentDepsT]) -> AbstractToolset[AgentDepsT]:
        """Return a fresh instance for one run's compatibility adapter.

        `get_toolset` builds one shared instance at agent construction. Per-run state keeps
        the lazily attached session isolated while the capability remains the sole lifecycle
        owner of the sandbox itself.
        """
    ...
    async def __aenter__(self) -> Self:
        ...
        With a caller-owned `session`, use it as-is. Otherwise the first tool call attaches
        a compatibility session to that call's `ctx.sandbox`; it never provisions another sandbox.
        """
    ...
    async def _require_session(self, ctx: RunContext[AgentDepsT]) -> ModalSandboxSession:
        """Return an adapter over this call's capability-provided sandbox.

        The released tools retain the session's bounded streaming implementation, but derive
        the attached sandbox ID from `ctx.sandbox`. This keeps the old tools and the new facade
        on one sandbox without making the compatibility layer another lifecycle owner.
        """
        if not self._run_scoped:
            raise ModalSandboxError('The Modal sandbox toolset is not active in an agent run.')
        sandbox_id = ctx.sandbox.sandbox_id
        if self._session is not None:
            if self._session.sandbox_id != sandbox_id:
                raise ModalSandboxError('The Modal compatibility tools and `ctx.sandbox` refer to different sandboxes.')
            return self._session
        async with self._session_lock:
            if self._session is None:
                session = ModalSandboxSession(sandbox_id=sandbox_id)
    ...
    async def run_command(
        ...
        Args:
            ctx: The current agent run, which supplies the shared sandbox.
