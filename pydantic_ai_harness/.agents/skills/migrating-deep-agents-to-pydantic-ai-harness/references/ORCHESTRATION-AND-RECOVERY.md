...
| --- | --- | --- |
| synchronous named children | `SubAgents` | Match child history, tools, dependencies, model selection, budgets, errors, events, and result serialization. |
...
- **Checkpoints:** separate message continuation, capability state, plans, files, approvals, domain state, and side effects. Use `StepPersistence` only for its documented snapshot contract.
