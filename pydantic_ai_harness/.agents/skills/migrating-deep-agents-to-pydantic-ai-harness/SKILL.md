...
2. Trace one representative request through the effective prompt and profile; tools, MCP, and retrieval; middleware and permissions; backend, sandbox, interpreter, and media paths; skills and memory; every subagent mode; state, checkpoints, approvals, grading, retries, events, limits, side effects, and the public result. Inspect every caller of the migrated boundary.
