...
Every run needs a sandbox attached: `LocalSandbox('.')` works on the checkout itself, and a [`ModalSandbox`](https://pydantic.dev/docs/ai/harness/modal-sandbox/) capability supplies a remote one. Interfaces that start runs for you, such as [`agent.to_cli_sync()`](https://pydantic.dev/docs/ai/cli/) and [`agent.to_web()`](https://pydantic.dev/docs/ai/web/), cannot attach one yet.
