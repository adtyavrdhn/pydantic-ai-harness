"""E2B sandbox toolset: shell and file tools over the run's sandbox."""
...
class E2BSandboxToolset(FunctionToolset[AgentDepsT]):
    ...
    the `E2BSandbox` capability (or by whatever else the run attached), read from `ctx.sandbox`
    on each call, so nothing here owns a connection or a lifetime.
    """
    ...
    def _command_timeout(self, timeout_seconds: float | None) -> float:
        if timeout_seconds is not None and (not math.isfinite(timeout_seconds) or timeout_seconds <= 0):
            # Reject rather than clamp: a 0 or negative request is a model mistake, and an
            # instant "[timed out]" would hide that from the model.
        ...
        # Clamp to a hard ceiling. It defaults to the sandbox lifetime, beyond which an owned
        # command cannot run anyway. The deadline is enforced client-side and E2B has no
        # whole-second constraint, so the requested value is used as given below the ceiling.
    ...
    async def run_command(  # noqa: D417
        ...
        The command runs through Bash, so pipes, redirection, `&&`, and globs
        ...
        except E2BSandboxCommandTimeoutError as e:
            # E2B's handle keeps what the command printed before the deadline kill; the
            # protocol's raise-on-timeout has nowhere to return it, so it rides on the
            # exception.
            return self._render(e.stdout, e.stderr, f'[timed out after {format_seconds(e.timeout)}s]')
        except TimeoutError:
            # Another backend's deadline kill, which reports no output alongside it.
            return f'[timed out after {format_seconds(deadline)}s]'
    ...
    async def read_file(  # noqa: D417
        ...
        *,
        ...
        try:
            ...
            raise
        except (FileNotFoundError, E2BSandboxError) as e:
